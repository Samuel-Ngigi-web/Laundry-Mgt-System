...
const connectionString = "mongodb://127.0.0.1:27017/TMLaundry";
...
app.post("/admin/loginsubmit", async function (req, res) {
  ...
  if (email === "admin10@gmail.com" && password === "admin1234") {
    const orderdata = await OrderModel.find({
      status: { $in: ["Processing", "Prepared", "Out for Deliver"] },
    });
...
app.post("/ordersubmit/:id", async (req, res) => {
  try {
    ...
    var TotalAmount = await (wp.shirts * ip.shirt +
      wp.pants * ip.pant +
      wp.jeans * ip.jean +
      wp.shorts * ip.short +
      wp.towels * ip.towel +
      wp.mundu * ip.sweatshirt +
      wp.bsheets * ip.bedsheet +
      wp.pillowc * ip.pillowcover +
      wp.tshirts * ip.tshirt);
